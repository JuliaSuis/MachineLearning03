...
def run(_store):
    ...
    data_column_names = ['RLAaccx',
                         'RLAaccy', 'RLAaccz']  # Right hand accelerometer; check dataset.py for feature names
    ...
    output_dir = '/home/devil/PycharmProjects/ML03/img/'  # Directory is needed for intermediate output when plotting
    ...
    test_seg = list(dataset.gen_seg_table(_store, _participants=test_participant, _labels=test_labels,
    ...
    test_df = dataset.get_frame(_store, _participant=test_participant, _start=test_min_start, _stop=test_max_stop,
    ...
    model.add_transitions(model_states, model_states[1:] + [model.end],
                          [0.2] * len(model_states))  # 20% to-next for each state
