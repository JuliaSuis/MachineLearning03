...
def _preformat_graph(_g, _graph=None, _grouped=None):
    ...
    for node in _g.nodes():
        ...
        node_colors[node] = ([lut[key] for key in lut if key in node.name] + ['white'])[
            0]  # Default to 'white' if no keyword found in node name
    ...
    for group in _grouped:
        seq_pairs.update({(u.identity, v.identity) for u, v in
                          zip(group[:-1], group[1:])})  # a -> b -> c -> d  TO  (a->b), (b->c), (c->d)
    edge_weights = {}
    for u, v in edges:
...
def from_sequence_make_graph(_indexed_state_seq, _drop_states=None, **_kwargs):
    ...
    state_seq = list(
        itertools.chain.from_iterable([itertools.repeat(u, n) for u, n in grouped_state_seq]))  # Rebuild state sequence
...
def from_model_make_graph(_model, **_kwargs):
    ...
    class DotState(collections.namedtuple("state", ['name', 'identity', 'distribution',
                                                    'hmm_weight'])):  # Custom named tuple for readable '.dot' files
        __slots__ = ()

        def __str__(self):
            return " ".join([self.name, ' ', self.identity])

    states = [DotState(name=s.name, identity=s.identity, distribution=s.distribution, hmm_weight=s.weight) for s in
              _model.states]
    mat_trans = _model.dense_transition_matrix()
    sparse_edges = [((states[n], states[m]), p) for n, x in enumerate(mat_trans) for m, p in enumerate(x) if
                    not math.isinf(p)]
    annotated_edges = map(lambda (e, log_p): e + ({'trans_weight': log_p},),
                          sparse_edges)  # (u, v, {'weight': probability}) edge tuples
...
def from_sequence_make_segments(_decoded, _model_to_label):
    ...
    for m, (ll, seq) in _decoded:
        ...
        def to_interval(_seq):
            start, _, _, model = itertools.ifilter(lambda (sample, n_state, state, model): sample is not None,
                                                   _seq).next()
            end, _, _, _ = itertools.ifilter(lambda (sample, n_state, state, model): sample is not None,
                                             reversed(_seq)).next()
