...
def run(_store):
    ...
    data_column_names = ['RLAaccx', 'RLAaccy', 'RLAaccz', 'RLAgyrx',
                         'RLAgyry', 'RLAgyrz']  # Right hand accelerometer; check dataset.py for feature names
    ...
    training_labels = [18, 19, 20, 21]  # Can pick from [18, 19, 20, 21]; see above
    ...
    test_labels = [19, 18, 20, 21]  # The labels associated with the gesture
    test_type = 'free-living'  # Can be initial 'training' data, 'free-living' data or 'both'
    test_seg = list(dataset.gen_seg_table(_store, _participants=test_participants, _labels=test_labels,
                                          _type=test_type))  # For for each subject
    assert test_seg  # Ensure we got data, otherwise the query was invalid
    test_seg = pd.concat([test_seg[0][1], test_seg[1][1], test_seg[2][1]],
                         ignore_index=True)  # Fetch single frame; there is only one
    # test_seg = test_seg.iloc[:10]  # Pick the first 10 segments
    test_min_start, test_max_stop = test_seg.iloc[0][0], test_seg.iloc[-1][
        1]  # Limit TEST data frame to region of interest
    test_min_start, test_max_stop = test_min_start - 300, test_max_stop + 300  # Add some margin for visibility
    test_df = pd.concat([dataset.get_frame(_store, _participant=test_participants[k], _start=test_min_start,
                                           _stop=test_max_stop, columns=data_column_names) for k in range(3)],
                        ignore_index=True)
    ...
    if False:
        ...
        lut = {key: col for key, col in label_to_color.iteritems() if
               key in test_labels}  # Reduce legend to used labels
    ...
    model1 = Model(name='Drink_tailored(syam)')
    ...
    # state_d = MultivariateDistribution([NormalDistribution(0, 2), NormalDistribution(0, 2), NormalDistribution(0, 2),
    #                                     ])
    # s2 = State(state_d, name=model.name + "_%d" % (2))
    #
    # state_d = MultivariateDistribution([NormalDistribution(0, 2), NormalDistribution(0, 2), NormalDistribution(0, 2),
    #                                     ])
    # s3 = State(state_d, name=model.name + "_%d" % (3))
    # state_d = MultivariateDistribution([NormalDistribution(0, 2), NormalDistribution(0, 2), NormalDistribution(0, 2),
    #                                     ])
    # s4 = State(state_d, name=model.name + "_%d" % (4))
    # state_d = MultivariateDistribution([NormalDistribution(0, 2), NormalDistribution(0, 2), NormalDistribution(0, 2),
    #                                     ])
    # s5 = State(state_d, name=model.name + "_%d" % (5))
    ...
    for i in range(3):
        s1 = State(make_dist(), name=model1.name + "_%d" % (i+1))
        model_states_1.append(s1)
    model1.add_states(model_states_1)
    model1.add_transition(model1.start, model_states_1[0], 1)  # Entry transition
    model1.add_transitions(model_states_1, model_states_1, [0.6] * len(model_states_1))  # 80% self loop for each state
    model1.add_transitions(model_states_1, model_states_1[1:] + [model1.end],
                           [0.4] * len(model_states_1))  # 20% to-next for each state

    for i in range(4):  # Generate some states
    ...
    model.add_transitions(model_states, model_states, [0.6] * len(model_states))  # 80% self loop for each state
    model.add_transitions(model_states, model_states[1:] + [model.end],
                          [0.4] * len(model_states))  # 20% to-next for each state
    ...
    # TODO: try this
    if True:  # Plot and save visual model
        plot_model(model, _output_file=output_dir + 'drink_initial.png',
                   _output_dpi=200)  # Check plot_graph() for more parameters
        plt.title('Initial gesture HMM')
        plt.show()

        plot_model(model1, _output_file=output_dir + 'drink_tailored(syam).png',
                   _output_dpi=200)  # Check plot_graph() for more parameters
        plt.title('Tailored gesture HMM')
        plt.show()
    ...
    model1.train(training_stream, algorithm='baum-welch', transition_pseudocount=1)
    ...
    backannotate_internal_graph(model1)

    # Plot and save visual model
    # TODO: try this
    if True:
        plot_model(model, _output_file=output_dir + 'drink_trained.png',
                   _output_dpi=200)  # Check plot_graph() for more parameters
        plt.title('Trained gesture HMM')
        plt.show()

        plot_model(model1, _output_file=output_dir + 'drink_tailored(syam)_trained.png',
                   _output_dpi=200)  # Check plot_graph() for more parameters
        plt.title('Trained tailored(syam) gesture HMM')
        plt.show()
    ...
    # TODO: try this
    if False:
        plot_model(model_tm, _output_file=output_dir + 'threshold_model.png',
                   _output_dpi=200)  # Check plot_graph() for more parameters
    ...
    model_tld.add_transition(model_tld_meta_entry, model.start, 0.2)
    model_tld.add_transition(model_tld_meta_entry, model1.start, 0.2)# Enter gesture model
    model_tld.add_transition(model_tm.end, model_tld_meta_exit, 1)
    model_tld.add_transition(model.end, model_tld_meta_exit, 1)
    model_tld.add_transition(model1.end, model_tld_meta_exit, 1)
    ...
    # TODO: try this
    if False:
        plot_model(model_tld, _output_file=output_dir + 'tld_model.png',
                   _output_dpi=200)  # Check plot_graph() for more parameters
    ...
    if False:
        for i, train_df in zip(range(3), training_dfs):
            ...
            test_decoded = from_data_make_sequence(train_df, _models=model_tld, _drop_states=drop_states,
                                                   _output_prefix=path, _show=True)
            ...
            lut = {key: col for key, col in label_to_color.iteritems() if
                   key in test_labels + [spotter_label]}  # Reduce legend to used labels
            add_labels_to_legend(_ax=ax, _label_to_color=lut)
    ...
    for i, (eval_df, eval_seg) in enumerate(zip(eval_dfs, eval_segs)):
        ...
        eval_decoded = from_data_make_sequence(eval_df, _models=model_tld, _drop_states=drop_states,
                                               _output_prefix=path, _show=True)
